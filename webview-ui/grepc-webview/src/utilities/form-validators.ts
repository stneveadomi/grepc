...
export class CSSValidator {
    ...
    public static classValidator(styleProperty: string): ValidatorFn {
        return (control: AbstractControl): ValidationErrors | null => {
            if(control.value && !this.validateCSS(styleProperty, control.value)) {
    ...
    static validateCSS(property: string, propertyValue: string) {
        ...
        switch(property) {
            ...
            case 'outline-width':
                ...
                break;
            case 'font-weight':
                return /^(normal|bold|bolder|lighter|100|200|300|400|500|600|700|800|900|inherit)$/.test(propertyValue);
            case 'font-style':
                return /^(normal|italic|oblique|inherit)$/.test(propertyValue);
            case 'text-decoration':
                return /^((none|underline|overline|line-through|blink)(\s+(solid|double|dotted|dashed|wavy))?(\s+\w+)?|inherit)$/.test(propertyValue);
            case 'cursor':
